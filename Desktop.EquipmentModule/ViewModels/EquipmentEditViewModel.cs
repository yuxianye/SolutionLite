...
namespace Desktop.EquipmentModule.ViewModels
{
    ...
    /// 编辑设备VM
    /// </summary>
    public class EquipmentEditViewModel : Desktop.Core.ViewModelBase
    {
        /// <summary>
        /// 构造函数
        /// </summary>
        ...
        protected override void OnParameterChanged(object sender, NotifyCollectionChangedEventArgs e)
        {
            ...
            {
                this.CurrentDataModel = (Equipment)(base.Parameter.FirstOrDefault(a => a.Key == DataModelName).Value as Equipment).Clone();
        ...
        private async void executeConfirmCommand()
        {
            ...
            try
            {
                ...
                if (dbContext.EquipmentDb.Update(CurrentDataModel))
                {
                    UiMessage = "编辑成功";
        ...
        private bool canExecuteConfirmCommand()
        {
            ...
            if (dbContext.EquipmentDb.Count(a => a.Code == CurrentDataModel.Code && a.Id != CurrentDataModel.Id) > 0)
            {
                UiMessage = $"设备编号【{CurrentDataModel.Code}】已被使用，请使用其他名称";
                return false;
            }

            if (dbContext.EquipmentDb.Count(a => a.Name == CurrentDataModel.Name && a.Id != CurrentDataModel.Id) > 0)
            {
