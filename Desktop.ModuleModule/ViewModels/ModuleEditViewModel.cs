...
namespace Desktop.ModuleModule.ViewModels
{
    ...
    /// 编辑模块VM
    /// </summary>
    public class ModuleEditViewModel : Desktop.Core.ViewModelBase
    {
        /// <summary>
        /// 构造函数
        /// </summary>
        ...
        protected override void OnParameterChanged(object sender, NotifyCollectionChangedEventArgs e)
        {
            ...
            {
                var module = (Module)((base.Parameter.FirstOrDefault(a => a.Key == DataModelName).Value as TreeGridItem)?.Value as Module)?.Clone();
                SelectedModule = module.ParentId == Guid.Empty ? topModule : dbContext.ModuleDb.GetSingle(a => a.Id == module.ParentId);
                CurrentDataModel.PropertyChanged -= CurrentDataModel_PropertyChanged;
                CurrentDataModel = module;
                CurrentDataModel.PropertyChanged += CurrentDataModel_PropertyChanged;
            }
        }
        ...
        private Module topModule = new Module() { Id = Guid.Empty, Name = "顶级菜单", ParentId = Guid.Empty };
        ...
        private void executeModuleSelectedCommand(TreeViewItem treeViewItem)
        {
            ...
            CurrentDataModel.ParentId = SelectedModule.Id;
        }
        ...
        private async void executeConfirmCommand()
        {
            ...
            try
            {
                ...
                if (dbContext.ModuleDb.Update(CurrentDataModel))
                {
                    UiMessage = "编辑成功";
        ...
        {
            if (dbContext.ModuleDb.Count(a => a.Name == CurrentDataModel.Name && a.Id != CurrentDataModel.Id) > 0)
            {
