...
namespace Desktop.NoticeModule.ViewModels
{
    ...
    /// 编辑通知VM
    /// </summary>
    public class NoticeEditViewModel : Desktop.Core.ViewModelBase
    {
        /// <summary>
        /// 构造函数
        /// </summary>
        public NoticeEditViewModel()
        {
            initCommand();
        }
        ...
        protected override void OnParameterChanged(object sender, NotifyCollectionChangedEventArgs e)
        {
            ...
            {
                this.CurrentDataModel = (Notice)(base.Parameter.FirstOrDefault(a => a.Key == DataModelName).Value as Notice).Clone();
        ...
        #region 当前的数据模型
        private Notice currentDataModel;
        /// <summary>
        /// 当前的数据模型
        /// </summary>
        public Notice CurrentDataModel
        {
        ...
        private async void executeConfirmCommand()
        {
            ...
            try
            {
                ...
                if (dbContext.NoticeDb.Update(CurrentDataModel))
                {
                    UiMessage = "编辑成功";
                    LogHelper.Logger.Info($"{StaticData.CurrentUser.Name }/{StaticData.CurrentUser.NickName},{UiMessage}{System.Environment.NewLine}{Utility.TypeExtensions.GetPropertiesValue<Notice>(CurrentDataModel)}");
